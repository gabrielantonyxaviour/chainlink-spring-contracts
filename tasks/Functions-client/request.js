...
task("functions-request", "Initiates a request from a Functions client contract")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const transactionEstimateGas = await clientContract.estimateGas.registerAccount(
      [],
    ...
    await new Promise(async (resolve, reject) => {
      ...
      clientContract.on("AccountRegistered", async (eventRequestId, claimer, emailAddress, timestamp) => {
      ...
      clientContract.on("ErrorOccured", async (eventRequestId, claimer, err) => {
        ...
        ocrResponseEventReceived = true
        await store.update(requestId, { status: "complete", result: err })
