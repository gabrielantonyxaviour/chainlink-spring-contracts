...
task("functions-simulate", "Simulates an end-to-end fulfillment locally for the BurnToEarn contract")
  ...
  .addOptionalParam("action", "Function to be called in the client contract (defaults to registerAccount)")
  .addOptionalParam(
    "configpath",
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    await new Promise(async (resolve) => {
      ...
      if (taskArgs.action == "register") {
        ...
        requestTx = await clientContract.mintTokens(request.secrets ?? [], subscriptionId, gasLimit)
      }
      ...
      if (taskArgs.action == "register") {
        ...
        client.on("AccountRegistered", async (eventRequestId, claimer, emailAddress, timestamp) => {
          ...
          if (emailAddress !== "0x") {
            console.log(`Response returned to client contract represented as a decoded string: ${emailAddress}\n`)
          }
        })
      } else {
        client.on("TokensMinted", async (eventRequestId, claimer, amount, timestamp) => {
          console.log("__Simulated On-Chain Response__")
          if (eventRequestId !== requestId) {
            throw new Error(`${eventRequestId} is not equal to ${requestId}`)
          }
          // Check for & log a successful request
          if (amount !== "0x") {
            console.log(`Response returned to client contract represented as a decoded uint: ${amount}\n`)
          }
        })
      }
      client.on("ErrorOccured", async (eventRequestId, claimer, err) => {
        console.log("__Simulated On-Chain Response__")
        if (eventRequestId !== requestId) {
          throw new Error(`${eventRequestId} is not equal to ${requestId}`)
        }
        console.log("Error returned for the request Id " + eventRequestId + " \nError: " + err)
      })
